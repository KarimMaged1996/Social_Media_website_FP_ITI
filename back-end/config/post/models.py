...
class Post(models.Model):
    ...
    )  # An optional image for the post
    ...
    video = models.FileField(
        upload_to="post_videos/", null=True, blank=True
    )  # An optional video for the post
    ...
    def update_score(self):
        ...
        self.score = Vote.objects.filter(
            post=self
        )  # => this will result on a queryset of all the votes
        print(self.score)
        # sum the column value
        self.score = Vote.objects.filter(post=self).aggregate(
            Sum("value")
        )  # => a dictionary of the {'value_sum': sum of the colume}
        print(self.score)
        # to get the value we have to bass the key which is ['value_sum']
        self.score = (
            Vote.objects.filter(post=self).aggregate(Sum("value"))["value__sum"] or 0
        )
        print(self.score)
        self.save()
